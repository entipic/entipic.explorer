...
external.update = function(dbUniqueName, entities) {
	...
	function updateTopic(dbTopic) {
		if (!dbTopic.englishWikiId && lastEntity.english && lastEntity.english.id) {
			var updateData = {
				id: dbUniqueName.topicId,
				englishWikiId: lastEntity.english.id,
				englishWikiDescription: lastEntity.english.description,
				englishWikiName: lastEntity.english.title
			};
			return Data.control.updateTopic(updateData)
				.then(function() {
					core.logger.warn('Updated topic english', _.pick(dbTopic, 'id', 'uniqueName', 'englishWikiName', 'englishWikiId'));
				})
				.catch(function(error) {
					core.logger.error('Error on updating topic english: ' + error.message, error);
				});
		}
	}

	return Data.access.topic({
			where: {
				_id: dbUniqueName.topicId
			}
		})
		.then(function(dbTopic) {
			return Promise.props({
					topic: updateTopic(dbTopic),
					names: updateNames()
				})
				.then(function() {});
		});
...
internal.saveTopic = function(topic, entities, picture) {
	...
	topic.popularity = 0;
	uniqueNames.forEach(function(un) {
		topic.popularity = un.popularity && un.popularity > topic.popularity ? un.popularity : topic.popularity;
	});
...
internal.savePictures = function(topic, pictures) {
	...
	return Promise.resolve(pictures).each(function(item) {
		...
		return Images.save(item.id, item.data)
			...
			.catch(function(e) {
				core.logger.error('Saving image: ' + e.message, e);
			});
	}).then(function() {
		if (!picture) {
			return Promise.reject(new errors.EntipicError({
				message: 'Not saved any tipic picture!',
				topic: topic
			}));
		}
		return picture;
...
internal.createTopicUniqueName = function(entity) {
	var uniqueNames = [];
	if (entity.english) {
		uniqueNames.push(Data.formatUniqueName({
			name: entity.english.title,
			lang: 'en',
			isLocal: false,
			wikiId: entity.english.id
		}));
		uniqueNames.push(Data.formatUniqueName({
			name: entity.english.title,
			lang: 'en',
