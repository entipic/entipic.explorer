...
external.update = function(dbUniqueName, entities) {
  ...
  core.logger.info('Updating topic: ' + dbUniqueName.uniqueName, dbUniqueName);
  ...
  //throw new Error('Aici');
};

external.create = function(entities) {
  var lastEntity = entities[entities.length - 1];
  var lang = lastEntity.name.lang;
  var country = lastEntity.name.country;
  var q = lastEntity.title || entities[0].name.name;

  // console.log('CREATE TOPIC from', lastEntity);
  // return;
  core.logger.info('Creating topic ' + lastEntity.name.name);

  return Promise.props({
      topic: internal.buildTopic(entities),
      pictures: internal.buildPictures(q, lang, country, lastEntity.type)
    })
    .then(function(props) {
...
internal.saveTopic = function(topic, entities, picture) {
  ...
  return Promise.map(uniqueNames, function(uname) {
    ...
    return Data.control.createTopic(topic);
  });
};

internal.savePictures = function(topic, pictures) {
  var picture = pictures[0];
  picture.topicId = topic.id;

  return Images.save(picture.id, picture.data)
    .then(function() {
    ...
    return Data.access.uniquename({
      ...
      .then(function(dbUniqueName) {
        if (!dbUniqueName) {
          ...
          un.status = 'selected';
          if (!exists) entity.uniqueNames.push(un);
        }
      });
  }).then(function() {
    return name;
...
internal.buildPictures = function(q, lang, country, type) {
  return imagesExplorer.explore(q, lang, country, {
      limit: 2,
      type: type === 'person' ? 'face' : 'photo'
    })
