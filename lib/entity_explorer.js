...
internal.createUniqueNames = function(entities) {
  ...
  if (lastEntity.id) {
    ...
    //   if (lastEntity.name.popularity > 1000000 && lastEntity.english.redirects) {
    //     lastEntity.english.redirects.forEach(function(rd) {
    //       if ((lastEntity.english.title.length / 2) < rd.title.length) {
    //         addUniqueName(entity, Data.formatUniqueName(rd.title, 'en', null, false, lastEntity.english.id));
    //       }
    //     });
    //   }
    // }
